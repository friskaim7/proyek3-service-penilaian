...
export const getKelasDosen = async (req, res, next) => {
  try {
    const nip = req.params.nip
    const error = validationResult(req)
    if (!error.isEmpty()) {
    ...
    const result = await DosenDAO.getKelasDosen(nip)
    res.status(200).json({
      message: 'Get Kelas Dosen sukses',
      data: {
        result
      }
    })
...
export const getMataKuliahDosen = async (req, res, next) => {
  try {
    const nip = req.params.nip
    const error = validationResult(req)
    if (!error.isEmpty()) {
    ...
    const result = await DosenDAO.getMataKuliahjDosen(nip)
    res.status(200).json({
      message: 'Get Mata Kuliah Dosen sukses',
      data: {
        result
      }
    })
